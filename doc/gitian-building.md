...
*Setup instructions for a Gitian build of Florincoin Core using a Debian VM or physical system.*
...
- [Building Florincoin Core](#building-florincoin-core)
...
Clone the git repositories for florincoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/florincoin-project/florincoin
...
in the florincoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Florincoin Core
----------------

To build Florincoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the florincoin repository.
...
For example:
```bash
URL=https://github.com/thrasher-/florincoin.git
...
and the florincoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../florincoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/florincoin
SIGPATH=/some/root/path/florincoin-detached-sigs

./bin/gbuild --url florincoin=${BTCPATH},signature=${SIGPATH} ../florincoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/florincoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/florincoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/florincoin-osx-build.assert
```
...
[gitian.sigs.ltc](https://github.com/florincoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
